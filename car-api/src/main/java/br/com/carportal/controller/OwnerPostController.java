...
public class OwnerPostController {
	...
	@GetMapping(
			produces = { MediaType.APPLICATION_JSON_VALUE })
	@Operation(summary = "Lista os Vendedores", description = "Recupera todos os vendedores",
	tags = {"Vendedor"},
	responses = {
			@ApiResponse(description = "Success", responseCode = "200",
					content = {
	...
	description = "Atualiza um vendedor/proprietário do carro através do seu identificador único",
	tags = {"Vendedor"},
	responses = {
	...
	public ResponseEntity<?> changeOwner(@RequestBody OwnerPostDTO ownerPostDTO, @PathVariable("id") String id ) {
		ownerPostService.changeOwner(ownerPostDTO, id);
		return ResponseEntity.ok().build();
	}
